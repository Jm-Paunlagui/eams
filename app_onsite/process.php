...
if (isset($_POST['time_in'])) {
    ...
    if ($validate === true) {
        ...
        if ($check_query = mysqli_query($db_connect, $check_sql)) {
            if (mysqli_num_rows($check_query) > 0) {
                ...
                if ($timein_val != NULL) {
                    ...
                    if (mysqli_query($db_connect, $log_sql)) {
                        $status = 'success';
...
function validate_user($card_id)
{
    global $db_connect;
    $id = $card_id;
    $validate_sql = "SELECT employee_id,card_id FROM users WHERE card_id='" . $id . "'";
    if ($validate_query = mysqli_query($db_connect, $validate_sql)) {
...
function fetchdata_user($card_id)
{
    global $db_connect;
    $id = $card_id;
    $validate_sql = "SELECT employee_id,f_name,l_name,card_id,position,img FROM users WHERE card_id='" . $id . "'";
    if ($validate_query = mysqli_query($db_connect, $validate_sql)) {
