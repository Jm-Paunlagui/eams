...
if (isset($_GET['table']) && $_GET['table'] == 'users') { // USERS
    ...
    // Users
    $table = "users";
    $dbfield = array('user_id', 'employee_id', 'card_id', 'f_name', 'm_name', 'l_name', 'suffix', 'user_role', 'username', 'email_address', 'position', 'status', 'locked', 'flag_employee');
    $dborig = array('user_id', 'employee_id', 'card_id', 'f_name', 'm_name', 'l_name', 'suffix', 'user_role', 'username', 'email_address', 'position', 'status', 'locked', 'flag_employee');
...
if (isset($_POST['actionSubmit']) && $_POST['actionSubmit'] == 'submitUser') {
    ...
    $table = 'users';
    $dbfield = array('employee_id', 'card_id', 'f_name', 'm_name', 'l_name', 'suffix', 'user_role', 'username', 'password', 'email_address', 'position', 'flag_employee');
    $insertfield = array();
...
if (isset($_POST['actionUpdate']) && $_POST['actionUpdate'] == 'submitUpdate') {
    ...
    $updatefield[] = "flag_employee = '" . $flag . "'";
...
if (isset($_POST['actionReset']) && $_POST['actionReset'] == 'submitReset') {
    ...
    $sql_where_array = array();
    $updatefield = array();
    $msg_reset = array("title" => "Successfully Reset Password","content_msg" =>$password_generate);
    ...
    $default_query = "UPDATE " . $table . " SET " . $field_query . " " . $sql_conds . " ";
    if ($query = call_mysql_query($default_query)) {
        $session_class->setValue('msg_reset', $msg_reset);
    } else {
        $session_class->setValue('msg_error', 'Unlocking Account Failed');
    }
    exit();
